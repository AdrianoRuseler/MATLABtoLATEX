%
\documentclass{standalone}
\usepackage{tikz}
\usepackage{pgfplots} % loads also tikz
\usepackage{grffile}
\pgfplotsset{compat=newest} % This will configure the compatibility layer
\usetikzlibrary{plotmarks}
\usetikzlibrary{pgfplots.groupplots}
\usepackage{amsmath}
\usepackage{color} % Using this package, you can set the font color, text background, or page background
\usetikzlibrary{arrows,shapes,positioning}
\usepackage{anyfontsize}
\usepackage{siunitx}

\tikzset{ % Set line width
	ultra thin/.style= {line width=0.1pt},
	very thin/.style=  {line width=0.2pt},
	thin/.style=       {line width=0.4pt},% thin is the default
	semithick/.style=  {line width=0.6pt},
	thick/.style=      {line width=0.8pt},
	very thick/.style= {line width=1.2pt},
	ultra thick/.style={line width=1.6pt}
}

\pgfplotsset{every axis legend/.style={
		cells={anchor=center},% Centered entries
		inner xsep=3pt,inner ysep=2pt,nodes={inner sep=2pt,text depth=0.15em},
		anchor=north east,
		shape=rectangle,
		fill=white,
		rounded corners,
		draw=black,
		at={(0.993,0.98)}
	}
}

\def \axiswidth {\linewidth} % Defines axis width value
\def \axisheight {0.5\linewidth} % Defines axis height value

% Defines color spec 
% http://www.color-hex.com for color details
\definecolor{c0color}{HTML}{00FF00} 
\definecolor{c1color}{HTML}{00FFFF}
\definecolor{c2color}{HTML}{0080FF}
\definecolor{c3color}{HTML}{0000FF}
\definecolor{c4color}{HTML}{8000FF}
\definecolor{c5color}{HTML}{FF00FF}
\definecolor{c6color}{HTML}{FF0080}
\definecolor{c7color}{HTML}{FF0000}
\definecolor{c8color}{HTML}{800000}
\definecolor{c9color}{HTML}{000000}

% 10 Shades of gray
\definecolor{c0gray}{HTML}{000000} 
\definecolor{c1gray}{HTML}{202120}
\definecolor{c2gray}{HTML}{363735}
\definecolor{c3gray}{HTML}{4b4d4a}
\definecolor{c4gray}{HTML}{4c4c4c}
\definecolor{c5gray}{HTML}{595959}
\definecolor{c6gray}{HTML}{666666}
\definecolor{c7gray}{HTML}{666666}
\definecolor{c8gray}{HTML}{474747}
\definecolor{c9gray}{HTML}{a3a3a3}


\begin{document}
  \begin{tikzpicture}
  
  \begin{groupplot}[group style={group size= 1 by 2, vertical sep=0.2cm,  horizontal sep=1cm},ylabel absolute]
\nextgroupplot[xmode=log,ymode=normal,height=\axisheight,width=\axiswidth,grid=major,
minor tick num=5,xtickten = {-10,...,10},xminorgrids=true,xticklabels=\empty,xlabel=\empty,
yticklabel style={/pgf/number format/fixed},ytick={-100, -50,   0,  50, 100},yticklabels={$-100$, $-50$, $0$, $50$, $100$},
xmin=0.01,xmax=10,ymin=-100,ymax=100,ylabel=Magnitude (dB),
] % End of axis configurations

% Settings for siunitx package
\sisetup{scientific-notation = fixed, fixed-exponent = 0, round-mode = places,round-precision = 2,output-decimal-marker = {,}}
\addplot[solid,thick,c1color]table[x=freq,y=mag,col sep=comma]{bodeH.csv}; % Add plot data
\addlegendentry{H};
\draw[thick] (6.2832,-100) -- (6.2832,100); % draw end of data line
\addplot[solid,thick,c2color]table[x=freq,y=mag,col sep=comma]{bodeHd.csv}; % Add plot data
\addlegendentry{Hd};
\draw[thick] (1e+20,-100) -- (1e+20,100); % draw end of data line

\nextgroupplot[xmode=log,ymode=normal,height=\axisheight,width=\axiswidth,grid=major,
minor tick num=5,xtickten = {-10,...,10},xminorgrids=true,xlabel=Frequ\^encia (rad/s),
yticklabel style={/pgf/number format/fixed},ytick={-270,-180, -90,   0},yticklabels={$-270$, $-180$, $-90$, $0$},
xmin=0.01,xmax=10,ymin=-272.7,ymax=2.7,ylabel=Fase (deg),
] % End of axis configurations

% Settings for siunitx package
\sisetup{scientific-notation = fixed, fixed-exponent = 0, round-mode = places,round-precision = 2,output-decimal-marker = {,}}
\addplot[solid,thick,c1color]table[x=freq,y=phase,col sep=comma]{bodeH.csv}; % Add plot data
\addlegendentry{H};
\draw[thick] (6.2832,-272.7) -- (6.2832,2.7); % draw end of data line
\addplot[solid,thick,c2color]table[x=freq,y=phase,col sep=comma]{bodeHd.csv}; % Add plot data
\addlegendentry{Hd};
\draw[thick] (1e+20,-272.7) -- (1e+20,2.7); % draw end of data line
    \end{groupplot}
  \end{tikzpicture}
\end{document} % End of file
